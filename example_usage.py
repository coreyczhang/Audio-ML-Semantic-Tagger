...
class AudioSemanticTagger:
    ...
    def visualize_pca(self, save_path='pca_visualization.png'):
        ...
        if hasattr(self, 'df') and hasattr(self, 'pca_object'):
            ...
            # Use stored PCA object to get loadings for top features
            # Get the indices of the top 15 features
            all_numeric_cols = [col for col in self.df.select_dtypes(include=[np.number]).columns 
                              if not col.startswith('PC') and col != 'semantic_cluster']
            
            loadings = self.pca_object.components_.T * np.sqrt(self.pca_object.explained_variance_)
            loading_matrix = pd.DataFrame(
                loadings[:15],
                columns=['PC1', 'PC2'],
                index=all_numeric_cols[:15]
            )
